...
const gateStore: Record<string, { count: number; resetAt: number }> = {}
...
router.get('/view', async (req, res) => {
  try {
    ...
    const paidCookie = (cookieHeader as string)
      .split(';')
      .map((c) => c.trim())
    ...
    if (paidCookie) {
      try {
        ...
        const expected = require('crypto').createHmac('sha256', secret).update(sid).digest('hex')
